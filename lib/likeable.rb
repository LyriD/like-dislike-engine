...
def add_like_nofrom(user, time = Time.now.to_f)
    uidkey = rand
    Likeable.redis.hset(like_key, uidkey, time)
    Likeable.redis.hset(user.like_key(self.class.to_s.downcase), uidkey, time)
    like = Like.new(:target => self, :user_id => uidkey, :time => time)
    after_like(like)
    clear_memoized_methods(:like_count, :like_user_ids, :liked_user_ids, :liked_users, :likes)
    like
end
...
def add_dislike_nofrom(user, time = Time.now.to_f)
    uidkey = rand
    Likeable.redis.hset(dislike_key, uidkey, time)
    Likeable.redis.hset(user.dislike_key(self.class.to_s.downcase), uidkey, time)
    dislike = Dislike.new(:target => self, :user_id => uidkey, :time => time)
    after_dislike(dislike)
    clear_memoized_methods(:dislike_count, :dislike_user_ids, :disliked_user_ids, :disliked_users, :dislikes)
    dislike
end
